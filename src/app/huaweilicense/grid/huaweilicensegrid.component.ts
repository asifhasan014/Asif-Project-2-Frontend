...
@Component({
  selector: 'app-huaweilicensegrid',
  templateUrl: './huaweilicensegrid.component.html',
  styleUrls: ['./huaweilicensegrid.component.css']
})
export class HuaweilicensegridComponent implements OnInit {

	gridOptions: GridOptions;
	huaweilicenses: Huaweilicense[];
	huaweilicenseList$;
	...
	constructor(
		...
		//this.huaweilicenseList$ = this.huaweilicenseService.getHuaweilicenseList();
		this.huaweilicenseList$ = this.huaweilicenseService.getHuaweilicensesByUniqueCodeAndDate(this.huaweilicense.uniqueCode, from, to);
		...
		this.gridOptions = <GridOptions> {
                ...
                this.huaweilicenseList$.pipe(
			      catchError(err => {
                    ...
                    apiResponse => {
						...
						this.loadHuaweilicensesIntoArray(apiResponse);
                        // the initial full set of data
                        // note that we don't need to un-subscribe here as it's a one off data load
                        if (this.gridOptions.api) { // can be null when tabbing between the examples
                            this.gridOptions.api.setRowData(this.huaweilicenses);
                        }
                        this.showSpinner =false;
                    }
                );
            ...
            onCellClicked: (event) => {
				if (event.column.getColId() === 'editAction') {
				  ...
				  router.navigate(['/huaweilicenses/' + selectedItemId]);
				}
			  }
			/*onSelectionChanged: () => {
				var selectedRows = this.gridOptions.api.getSelectedRows();
	...
	private loadHuaweilicensesIntoArray(apiResponse){
		if (!apiResponse.success){
			return;
		}
		
		this.huaweilicenses = apiResponse.data.map(obj =>{
			var rObj = <Huaweilicense>{
				...
				version: obj.version ,
					...
					authorizedCapacity: obj.authorizedCapacity,
	...
	onAddHuaweilicense(){
		this.router.navigate(['/huaweilicenses/-1']);
	}
	
	/* searchByParams(){
		this.showSpinner =true;
		this.huaweilicenseList$ = this.huaweilicenseService.getHuaweilicensesByUniqueCode(this.huaweilicense.uniqueCode);
		this.huaweilicenseList$.pipe(
		      catchError(err => {
		        this.alertService.error(err);
		        this.showSpinner = false;
	...
	downloadReport(){
		...
		if (from.length > 0 || to.length > 0) {
			this.showSpinner =true;
			let finalRequestParam = "?uniqueCode="+this.huaweilicense.uniqueCode+"&from=" + from + "&to=" + to
				+"&fromTime="+fromTime+"&toTime="+toTime;
			this.huaweilicenseService.downloadReport(finalRequestParam).subscribe(response => {
				let blob: any = new Blob([response.blob()], {type: 'text/csv; charset=utf-8'});
				saveAs(blob, 'huaweilicense Report.csv');			
				this.showSpinner=false;
			}, err => {
			...
			this.showSpinner =true;			
			let finalRequestParam1 = "?uniqueCode="+this.huaweilicense.uniqueCode;
			this.huaweilicenseService.downloadReport(finalRequestParam1).subscribe(response => {
				let blob:any = new Blob([response.blob()], { type: 'text/csv; charset=utf-8' });
				saveAs(blob, 'Huaweilicense Report.csv');
				this.showSpinner=false;
			}, err => {
	...
	private createColumnDefs(){
		return [
			...
			{
				headerName: "Authorized Capacity",
