...
@Component({
   selector: "app-mwdcndashboardconfigurationgrid",
   templateUrl: "./mwdcndashboardconfigurationgrid.component.html",
   styleUrls: ["./mwdcndashboardconfigurationgrid.component.css"],
})
export class MwdcndashboardconfigurationgridComponent implements OnInit {
   gridOptions: GridOptions;
   mwdcndashboardconfigurations: Mwdcndashboardconfiguration[];
   mwdcndashboardconfigurationList$;
   ...
   constructor(
      ...
      this.mwdcndashboardconfigurationList$ = this.mwdcndashboardconfigurationService.getMwdcndashboardconfigurationList();
      //   this.mwdcndashboardconfigurationList$ = this.mwdcndashboardconfigurationService.getMwdcndashboardconfigurationsByUniqueCodeAndDate(
      ...
      this.gridOptions = <GridOptions>{
         ...
         onGridReady: () => {
            this.mwdcndashboardconfigurationList$
               .pipe(
                  catchError((err) => {
               ...
               .subscribe((apiResponse) => {
                  ...
                  this.loadMwdcndashboardconfigurationsIntoArray(apiResponse);
                  // the initial full set of data
                  // note that we don't need to un-subscribe here as it's a one off data load
                  if (this.gridOptions.api) {
                     // can be null when tabbing between the examples
                     this.gridOptions.api.setRowData(
                        this.mwdcndashboardconfigurations
                     );
                  }
                  this.showSpinner = false;
               });
         ...
         onCellClicked: (event) => {
            if (event.column.getColId() === "editAction") {
               ...
               router.navigate([
                  "/mwdcndashboardconfigurations/" + selectedItemId,
               ]);
            }
         },
         /*onSelectionChanged: () => {
				...
				router.navigate(['/mwdcndashboardconfigurations/' + selectedItemId]);
			}*/
      };
   }
   ...
   private loadMwdcndashboardconfigurationsIntoArray(apiResponse) {
      if (!apiResponse.success) {
         return;
      }

      this.mwdcndashboardconfigurations = apiResponse.data.map((obj) => {
   ...
   onAddMwdcndashboardconfiguration() {
      this.router.navigate(["/mwdcndashboardconfigurations/-1"]);
   }

   /* searchByParams(){
		this.showSpinner =true;
		this.mwdcndashboardconfigurationList$ = this.mwdcndashboardconfigurationService.getMwdcndashboardconfigurationsByUniqueCode(this.mwdcndashboardconfiguration.uniqueCode);
		this.mwdcndashboardconfigurationList$.pipe(
		      catchError(err => {
		        this.alertService.error(err);
		        this.showSpinner = false;
		        return throwError(err);
			...
			apiResponse => {
				...
				if (this.gridOptions.api) { // can be null when tabbing between the examples
					this.gridOptions.api.setRowData(this.mwdcndashboardconfigurations);
				}
				this.showSpinner =false;
			}
			);
   ...
   downloadReport() {
      ...
      if (from.length > 0 || to.length > 0) {
         ...
         this.mwdcndashboardconfigurationService
            ...
            .subscribe(
               (response) => {
                  ...
                  saveAs(blob, "mwdcndashboardconfiguration Report.csv");
                  this.showSpinner = false;
               },
               (err) => {
         ...
         let finalRequestParam1 =
            "?uniqueCode=" + this.mwdcndashboardconfiguration.uniqueCode;
         this.mwdcndashboardconfigurationService
