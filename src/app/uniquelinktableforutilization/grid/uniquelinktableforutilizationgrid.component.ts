...
@Component({
  selector: 'app-uniquelinktableforutilizationgrid',
  templateUrl: './uniquelinktableforutilizationgrid.component.html',
  styleUrls: ['./uniquelinktableforutilizationgrid.component.css']
})
export class UniquelinktableforutilizationgridComponent implements OnInit {

	gridOptions: GridOptions;
	uniquelinktableforutilizations: Uniquelinktableforutilization[];
	uniquelinktableforutilizationList$;
	...
	constructor(
		...
		//this.uniquelinktableforutilizationList$ = this.uniquelinktableforutilizationService.getUniquelinktableforutilizationList();
		this.uniquelinktableforutilizationList$ = this.uniquelinktableforutilizationService.getUniquelinktableforutilizationsByUniqueCodeAndDate(this.uniquelinktableforutilization.uniqueCode, from, to);
		...
		this.gridOptions = <GridOptions> {
            ...
            onGridReady: () => {
                this.uniquelinktableforutilizationList$.pipe(
			      catchError(err => {
                    ...
                    apiResponse => {
						...
						this.loadUniquelinktableforutilizationsIntoArray(apiResponse);
                        // the initial full set of data
                        // note that we don't need to un-subscribe here as it's a one off data load
                        if (this.gridOptions.api) { // can be null when tabbing between the examples
                            this.gridOptions.api.setRowData(this.uniquelinktableforutilizations);
                        }
                        this.showSpinner =false;
                    }
                );
            ...
            onCellClicked: (event) => {
				if (event.column.getColId() === 'editAction') {
				  ...
				  router.navigate(['/uniquelinktableforutilizations/' + selectedItemId]);
				}
			  }
			/*onSelectionChanged: () => {
				var selectedRows = this.gridOptions.api.getSelectedRows();
	...
	private loadUniquelinktableforutilizationsIntoArray(apiResponse){
		if (!apiResponse.success){
			return;
		}
		
		this.uniquelinktableforutilizations = apiResponse.data.map(obj =>{
			var rObj = <Uniquelinktableforutilization>{
	...
	onAddUniquelinktableforutilization(){
		this.router.navigate(['/uniquelinktableforutilizations/-1']);
	}
	
	/* searchByParams(){
		this.showSpinner =true;
		this.uniquelinktableforutilizationList$ = this.uniquelinktableforutilizationService.getUniquelinktableforutilizationsByUniqueCode(this.uniquelinktableforutilization.uniqueCode);
		this.uniquelinktableforutilizationList$.pipe(
		      catchError(err => {
		        this.alertService.error(err);
		        this.showSpinner = false;
	...
	downloadReport(){
		...
		if (from.length > 0 || to.length > 0) {
			this.showSpinner =true;
			let finalRequestParam = "?uniqueCode="+this.uniquelinktableforutilization.uniqueCode+"&from=" + from + "&to=" + to
				+"&fromTime="+fromTime+"&toTime="+toTime;
			this.uniquelinktableforutilizationService.downloadReport(finalRequestParam).subscribe(response => {
				let blob: any = new Blob([response.blob()], {type: 'text/csv; charset=utf-8'});
				saveAs(blob, 'uniquelinktableforutilization Report.csv');			
				this.showSpinner=false;
			}, err => {
			...
			this.showSpinner =true;			
			let finalRequestParam1 = "?uniqueCode="+this.uniquelinktableforutilization.uniqueCode;
			this.uniquelinktableforutilizationService.downloadReport(finalRequestParam1).subscribe(response => {
				let blob:any = new Blob([response.blob()], { type: 'text/csv; charset=utf-8' });
				saveAs(blob, 'Uniquelinktableforutilization Report.csv');
				this.showSpinner=false;
			}, err => {
