...
import { UnusedandmostusedrulereportService } from '../../service/unusedandmostusedrulereport.service';

@Component({
  selector: 'app-unusedandmostusedrulereport',
  templateUrl: './unusedandmostusedrulereport.component.html',
  styleUrls: ['./unusedandmostusedrulereport.component.css']
})
export class UnusedandmostusedrulereportComponent implements OnInit {
  ...
  gridOptions: GridOptions;
	...
	constructor(
		...
		private unnusedandmostusedrulereportService: UnusedandmostusedrulereportService,
	...
	downloadReport(){
		...
		if (from.length > 0 || to.length > 0) {
			...
			this.unnusedandmostusedrulereportService.downloadReportByParam(finalRequestParam).subscribe(response => {
        ...
        console.log(response)
			...
			this.unnusedandmostusedrulereportService.downloadReportByParam(finalRequestParam1).subscribe(response => {
				let blob:any = new Blob([response.blob()], { type: 'text/csv; charset=utf-8' });
				saveAs(blob, 'Unused & Most Used Rule Report.csv');
	...
	downloadReportdenyHit(){
		...
		if (from.length > 0 || to.length > 0) {
			...
			let finalRequestParam = "?uniqueCode="+this.fsadisplaysecuritypolicyruleall.uniqueCode+"&from=" + from + "&to=" + to
				+"&fromTime="+fromTime+"&toTime="+toTime;
			this.unnusedandmostusedrulereportService.downloadReportByParamdenyHit(finalRequestParam).subscribe(response => {
				let blob: any = new Blob([response.blob()], {type: 'text/csv; charset=utf-8'});
			...
			this.showSpinner =true;
			let finalRequestParam1 = "?uniqueCode="+this.fsadisplaysecuritypolicyruleall.uniqueCode;
			this.unnusedandmostusedrulereportService.downloadReportByParamdenyHit(finalRequestParam1).subscribe(response => {
				let blob:any = new Blob([response.blob()], { type: 'text/csv; charset=utf-8' });
				saveAs(blob, 'Deny Hit Report.csv');			
				this.showSpinner=false;
			}, err => {
	...
	searchByParams(){
		...
		this.showSpinner =true;
		this.fsadisplaysecuritypolicyruleallList$ = this.unnusedandmostusedrulereportService.getFsadisplaysecuritypolicyruleallsByUniqueCode(this.fsadisplaysecuritypolicyruleall.uniqueCode+","+from+","+to+",end");
  ...
  search(){
		...
		this.showSpinner =true;
		this.fsadisplaysecuritypolicyruleallList$ = this.unnusedandmostusedrulereportService.getDataforUnusedMostUsed(from, to, this.fsadisplaysecuritypolicyruleall.uniqueCode);
