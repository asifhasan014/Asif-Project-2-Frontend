...
@Component({
	selector: 'app-utilisationericssonlangrid',
	templateUrl: './utilisationericssonlangrid.component.html',
	styleUrls: ['./utilisationericssonlangrid.component.css']
})
export class UtilisationericssonlangridComponent implements OnInit {

	gridOptions: GridOptions;
	utilisationericssonlans: Utilisationericssonlan[];
	utilisationericssonlanList$;
	...
	constructor(
		...
		//this.utilisationericssonlanList$ = this.utilisationericssonlanService.getUtilisationericssonlanList();
		this.utilisationericssonlanList$ = this.utilisationericssonlanService.getUtilisationericssonlansByUniqueCodeAndDate(this.utilisationericssonlan.uniqueCode, from, to);
		...
		this.gridOptions = <GridOptions>{
			...
			rowSelection: 'single',
			onGridReady: () => {
				this.utilisationericssonlanList$.pipe(
					catchError(err => {
					...
					apiResponse => {
						...
						this.loadUtilisationericssonlansIntoArray(apiResponse);
						// the initial full set of data
						// note that we don't need to un-subscribe here as it's a one off data load
						if (this.gridOptions.api) { // can be null when tabbing between the examples
							this.gridOptions.api.setRowData(this.utilisationericssonlans);
						}
						this.showSpinner = false;
					}
				);
			...
			onSelectionChanged: () => {
				...
				router.navigate(['/utilisationericssonlans/' + selectedItemId]);
			}
		};
	...
	private loadUtilisationericssonlansIntoArray(apiResponse) {
		if (!apiResponse.success) {
			return;
		}

		this.utilisationericssonlans = apiResponse.data.map(obj => {
			var rObj = <Utilisationericssonlan>{
	...
	onAddUtilisationericssonlan() {
		this.router.navigate(['/utilisationericssonlans/-1']);
	}

	/* searchByParams(){
		this.showSpinner =true;
		this.utilisationericssonlanList$ = this.utilisationericssonlanService.getUtilisationericssonlansByUniqueCode(this.utilisationericssonlan.uniqueCode);
		this.utilisationericssonlanList$.pipe(
			  catchError(err => {
				this.alertService.error(err);
				this.showSpinner = false;
	...
	downloadReport() {
		...
		if (from.length > 0 || to.length > 0) {
			this.showSpinner = true;
			let finalRequestParam = "?uniqueCode=" + this.utilisationericssonlan.uniqueCode + "&from=" + from + "&to=" + to
				+ "&fromTime=" + fromTime + "&toTime=" + toTime;
			this.utilisationericssonlanService.downloadReport(finalRequestParam).subscribe(response => {
				let blob: any = new Blob([response.blob()], { type: 'text/csv; charset=utf-8' });
				saveAs(blob, 'utilisationericssonlan Report.csv');
				this.showSpinner = false;
			}, err => {
			...
			this.showSpinner = true;
			let finalRequestParam1 = "?uniqueCode=" + this.utilisationericssonlan.uniqueCode;
			this.utilisationericssonlanService.downloadReport(finalRequestParam1).subscribe(response => {
				let blob: any = new Blob([response.blob()], { type: 'text/csv; charset=utf-8' });
				saveAs(blob, 'Utilisationericssonlan Report.csv');
				this.showSpinner = false;
			}, err => {
